...
GOOS=linux go build -ldflags "-X ${PKG}/pkg/version.buildVersion=${tag}" -o $PKG/bin/linux_amd64/mysql-agent $PKG/cmd/mysql-agent
GOOS=linux go build -ldflags "-X ${PKG}/pkg/version.buildVersion=${tag}" -o $PKG/bin/linux_amd64/mysql-operator $PKG/cmd/mysql-operator
